...
function CampaignLoaded(props: CampaignProps): ReactElement {
  const {
    campaign, freeText: freeTextProp, brief: briefProp,
    compileResult: compileResultProp, emptySetAcknowledged: emptySetAckProp,
    buildPhase = "idle", buildInFlight = false, missingFields = [], campaignAssets = [],
    staleBanner: staleBannerProp, s2Dimmed: s2DimmedProp, s3Dimmed: s3DimmedProp,
    briefDirty: briefDirtyProp, briefSaved: briefSavedProp,
    runningStep: runningStepProp, initialCompileResult = null, zeroRulesCompile = false,
    onFreeTextChange, onBriefChange, onFieldEdit, onEmptySetAckChange,
    onCompile, onGenerate, onRunBuild,
  } = props;

  const fixture = useCampaignFixture(campaign, initialCompileResult, zeroRulesCompile);
  ...
  const brief = controlled ? (briefProp ?? briefFromCampaign(null)) : fixture.brief;
  const compileResult = controlled ? (compileResultProp ?? null) : fixture.compileResult;
  const emptySetAcknowledged = controlled ? (emptySetAckProp ?? false) : fixture.emptySetAcknowledged;

  const fixtureGate = campaignGateState({
    brief: fixture.brief, savedBrief: fixture.savedBrief, briefSaved: fixture.briefSaved,
    compileResult: fixture.compileResult, emptySetAcknowledged: fixture.emptySetAcknowledged,
    generateInFlight: false,
  });

  const staleBanner = controlled ? (staleBannerProp ?? false) : fixtureGate.staleBanner;
  ...
  const pane = useMemo(
    () => activeCampaignPane({ hasAssets, buildInFlight, buildPhase, paneOverride, railView }),
    [hasAssets, buildInFlight, buildPhase, paneOverride, railView],
  );

  const activeStep = runningStepProp ?? railStepForPane(pane);
  const showBack = paneOverride !== null && (pane === "brief" || pane === "freeze");
  ...
  const isBriefSaved = controlled ? (briefSavedProp ?? false) : fixture.briefSaved;
  const isBriefDirty = controlled ? (briefDirtyProp ?? false) : fixtureGate.briefDirty;
  const documentState: "Draft" | "Saved" | "Compiled" =
    compileResult !== null ? "Compiled" : (isBriefSaved && !isBriefDirty ? "Saved" : "Draft");

  const sharedBriefProps = {
    freeText, brief, documentState,
    ...
    buildPhase, buildInFlight, missingFieldsBuild: missingFields, emptySetAcknowledged,
  ...
  if (pane === "building" && buildPhase === "generate" && compileResult !== null) {
    paneContent = (
      <GeneratingPane
        buildPhase={buildPhase} buildInFlight={buildInFlight}
        compileResult={compileResult} onRunBuild={onRunBuild ?? (() => undefined)}
      />
    );
    ...
    paneContent = (
      <BuiltSummary
        brief={brief} compileResult={compileResult} assets={campaignAssets}
        onEditBrief={() => setPaneOverride("brief-edit")} onViewFreeze={() => setPaneOverride("freeze")}
      />
    );
  } else {
  ...
  return (
    <CampaignPageShell
      ...
      onRailStepChange={(stepId) => {
        setPaneOverride(null);
        setRailView(stepId);
        if (stepId === "campaign-constraints" && !s2Dimmed && compileResult === null && onCompile) {
          onCompile();
        }
        if (stepId === "campaign-generate" && !s3Dimmed && !hasAssets && onGenerate) {
          onGenerate();
        }
      }}
    >
...
export function Campaign(props: CampaignProps): ReactElement {
  ...
  if (view === "loading") return <CampaignLoadingState showSpinner={showLoadingSpinner} />;
  if (view === "error") return <CampaignErrorState onRetry={onRetry} />;
